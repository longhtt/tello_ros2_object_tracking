...
{
  // Subscribers
  _img_sub = create_subscription<sensor_msgs::msg::Image>("/image_raw", rclcpp::SensorDataQoS(), bind(&Tracker::_imageCallback, this, _1));
...
void Tracker::_imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
{
  ...
  // Assuming 'obj' is of type cv::Rect and 'frame' is your image
    cv::Rect2d obj2d(obj.x, obj.y, obj.width, obj.height);
    bool ok = _tracker->update(frame, obj2d);
...
void Tracker::_designateControl(geometry_msgs::msg::Twist &vel_msg, cv::Rect2d obj, uint32_t img_width)
{
    ...
    */
    ...
    // Determine if the object is left or right of the screen center and set angular velocity accordingly
    if (obj_x_center < screen_center_x) {
        // Object is to the left of the center
        vel_msg.angular.z = 1.0;
    } else if (obj_x_center > screen_center_x) {
    ...
    RCLCPP_INFO(this->get_logger(), "obj_x: %d, obj_width: %d, obj_x_center: %d, img_width: %d, screen_center_x: %d, angular.z: %f", 
                obj.x, obj.width, obj_x_center, img_width, screen_center_x, vel_msg.angular.z);
}
